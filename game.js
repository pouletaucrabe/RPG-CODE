...
function playRoiIntro(mob, tierMob) {
  ...
  setTimeout(() => {
    ...
    cf.style.background = "rgba(120,0,0,0.5)"
    const box = document.getElementById("storyImage")
    const img = document.getElementById("storyImageContent")
    if (box && img) {
    ...
    const roiSnd = new Audio("audio/roi.mp3")
    roiSnd.volume = 1.0
    roiSnd.play().catch(() => {})
    setTimeout(() => {
      const iv = setInterval(() => {
        if (roiSnd.volume > 0.04) roiSnd.volume = Math.max(0, roiSnd.volume - 0.05)
        else {
          roiSnd.pause()
          clearInterval(iv)
        }
    ...
    setTimeout(() => {
      ...
      cf.style.transition = "opacity 1s ease"
      cf.style.opacity = "0"
    }, 15000)
    setTimeout(() => {
      if (box) {
...
function fadeToCombat() {
  ...
  ;["greg","ju","elo","bibi"].forEach((id, i) => {
    ...
    token.style.width = "70px"
    token.style.height = "70px"
...
function showVictory() {
  ...
  setTimeout(() => {
    ...
    if (vs) {
      const iv = setInterval(() => {
        if (vs.volume > 0.03) vs.volume -= 0.03
        else {
          vs.pause()
          vs.volume = 0
          clearInterval(iv)
...
function stopBossFireEffect() {
  const overlay = document.getElementById("bossFireOverlay")
  if (!overlay) return
...
function playOpeningCinematic(callback) {
  ...
  const tremb = new Audio("audio/tremblement.mp3")
  tremb.volume = 0.8
  tremb.play().catch(() => {})
  setTimeout(() => {
    const iv = setInterval(() => {
      if (tremb.volume > 0.05) tremb.volume -= 0.05
      else {
        tremb.pause()
        clearInterval(iv)
      }
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ;["fart.mp3","cribibi.mp3","ah.mp3"].forEach((s, i) => {
        setTimeout(() => {
          const snd = new Audio("audio/" + s)
          snd.volume = 0
          snd.play().catch(() => {})
          const inIv = setInterval(() => {
            if (snd.volume < 0.55) snd.volume = Math.min(0.55, snd.volume + 0.04)
            else clearInterval(inIv)
          }, 80)
          setTimeout(() => {
            const outIv = setInterval(() => {
              if (snd.volume > 0.03) snd.volume = Math.max(0, snd.volume - 0.04)
              else {
                snd.pause()
                clearInterval(outIv)
              }
      ...
      const gb = new Audio("audio/grisebarbe.mp3")
      gb.volume = 0
      gb.play().catch(() => {})
      const gbIn = setInterval(() => {
        if (gb.volume < 0.7) gb.volume = Math.min(0.7, gb.volume + 0.03)
        else clearInterval(gbIn)
      }, 100)
      setTimeout(() => {
        const gbOut = setInterval(() => {
          if (gb.volume > 0.02) gb.volume -= 0.03
          else {
            gb.pause()
            clearInterval(gbOut)
          }
  ...
  setTimeout(() => {
    ...
    t.style.cssText = "font-family:'Cinzel Decorative','Cinzel',serif;font-size:18px;letter-spacing:5px;color:#c8a050;text-shadow:0 0 15px gold;text-align:center;margin-bottom:14px;"
    t.innerText = "Prophétie des Enfants de Mouches"
    wrapper.appendChild(t)
    const v = document.createElement("div")
    v.style.cssText = "font-family:'IM Fell English',serif;font-size:15px;color:rgba(200,160,50,0.8);line-height:2;text-align:center;font-style:italic;"
    v.innerText = "— Livre I, Verset 1 —"
    wrapper.appendChild(v)
    screen.appendChild(wrapper)
    setTimeout(() => { wrapper.style.opacity = "1" }, 50)
    const prop = new Audio("audio/prophetie.mp3")
    prop.volume = 0
    prop.play().catch(() => {})
    const pIv = setInterval(() => {
      if (prop.volume < 0.8) prop.volume = Math.min(0.8, prop.volume + 0.04)
      else clearInterval(pIv)
    }, 100)
  }, 21000)

  setTimeout(() => {
    screen.style.transition = "opacity 2s ease"
...
function toggleMobDropdown(slot, triggerEl) {
  const dd = document.getElementById("mobDropdown_" + slot)
  if (!dd) return
  ...
  if (!dd.dataset.built) {
    ...
    none.style.cssText = "padding:5px 10px;font-family:Cinzel,serif;font-size:11px;color:rgb(180,100,100);cursor:pointer;"
    none.innerText = "— Aucun —"
    none.onmousedown = e => {
      e.stopPropagation()
      selectMobOption(slot, "", "— Aucun —")
    }
    dd.appendChild(none)
    MOB_SELECT_LIST.forEach(m => {
      const item = document.createElement("div")
      item.style.cssText = "padding:5px 10px;font-family:Cinzel,serif;font-size:11px;color:rgb(255,180,180);cursor:pointer;"
      item.innerText = m.charAt(0).toUpperCase() + m.slice(1)
      item.onmousedown = e => {
        e.stopPropagation()
        selectMobOption(slot, m, item.innerText)
      }
