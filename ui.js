...
function showAuroraEvent() {
  ...
  fadeMusicOut(()=>{ const mA=document.getElementById("musicA"); if(mA){ mA.src="audio/aurore.mp3"; mA.loop=true; mA.volume=0; mA.currentTime=0; mA.play().catch(()=>{}); let iv=setInterval(()=>{ if(mA.volume<0.35) mA.volume=Math.min(0.35,mA.volume+0.03); else clearInterval(iv) },100); currentMusic="A" } })
