...
function openCharacterSheet(id = null) {
  ...
  document.getElementById("sheetImage").src = "" + (portraits[playerID] || "elosheet.jpg")
...
function updateWeightBar() {
  ...
  db.ref("characters/" + currentSheetPlayer + "/poids").once("value", snap => {
    ...
    const pct = Math.min(100, (total/max)*100), bar = document.getElementById("weightBar")
    bar.style.width = pct + "%"; bar.style.background = pct < 70 ? "lime" : pct < 100 ? "orange" : "red"
  })
}
...
function showCombatHUD() {
  ...
  document.getElementById("combatHUDPortrait").src   = "" + player + ".png"
...
function openGMPlayerSheet(playerID) {
  ...
  title.innerHTML = '<img class="gmMiniToken" src="'+playerID+'.png">'+playerID.toUpperCase(); box.appendChild(title)
...
function spawnExtraMobToken(mobData, slot) {
  ...
  const img=document.createElement("img"); img.style.cssText="width:60px;height:60px;object-fit:contain;border-radius:50%;border:2px solid #cc2200;box-shadow:0 0 10px rgba(200,0,0,0.5);"; img.src=""+mobData.name+".png"; img.onerror=()=>img.style.display="none"; tok.appendChild(img)
...
function showStoryImage(image) {
  ...
  img.src=""+image; box.style.opacity="0"; box.style.left="50%"; box.style.transform="translateX(-50%)"; box.style.right="auto"; box.style.display="flex"
  ...
  if(pnjSounds[image]){ const sid="pnjSound_"+image.replace(/[^a-z]/g,""); let snd=document.getElementById(sid); if(!snd){ snd=document.createElement("audio"); snd.id=sid; snd.src=""+pnjSounds[image]; snd.volume=1.0; document.body.appendChild(snd) }; snd.currentTime=0; snd.play().catch(()=>{}); setTimeout(()=>{ let iv=setInterval(()=>{ if(snd.volume>0.05) snd.volume-=0.05; else{ snd.pause(); snd.volume=1.0; clearInterval(iv) } },100) },2000) }
}
...
function _buildShop(partyLvl, runeCard, activeItems, shopTitle) {
  ...
  const box=document.createElement("div"); box.style.cssText="background:url('shelf.png') center/100% 100% no-repeat;width:min(900px,92vw);max-height:85vh;overflow-y:auto;padding:50px 50px 40px 50px;min-height:400px;position:relative;font-family:'IM Fell English',serif;"
...
function closeRuneChallenge() { db.ref("game/runeChallenge").remove(); const ov=document.getElementById("runeChallengeOverlay"); if(ov) ov.remove() }
...
function renderRuneChallenge(data) {
  ...
  const mb=document.createElement("div"); mb.style.cssText="background:url('roc.png') center/100% 100% no-repeat;padding:50px 70px;max-width:700px;width:90vw;text-align:center;margin-bottom:24px;border-radius:4px;"
  ...
  if(isGM){ const as=document.createElement("div"); as.style.cssText="display:flex;flex-direction:column;align-items:center;gap:10px;width:90vw;max-width:600px;"; const ai=document.createElement("input"); ai.placeholder="Tapez votre réponse ici..."; ai.style.cssText="width:100%;padding:12px 20px;font-family:'Cinzel',serif;font-size:14px;background:rgba(200,160,80,0.1);border:1px solid rgba(200,160,80,0.4);border-radius:4px;color:#f5e6c8;text-align:center;outline:none;"; const sb=document.createElement("button"); sb.innerText="⚔ Valider la réponse"; sb.style.cssText="padding:10px 30px;font-family:'Cinzel',serif;font-size:14px;background:linear-gradient(#7a5520,#3a2508);color:#c8a050;border:1px solid #c8a050;border-radius:4px;cursor:pointer;"; sb.onclick=()=>{ const ans=ai.value.toLowerCase().replace(/[^a-zéèàâêôîûçœ ]/g,"").replace(/\s+/g," ").trim(), tgt=secretAnswer.replace(/[^a-zéèàâêôîûçœ ]/g,"").replace(/\s+/g," ").trim(); if(ans===tgt) showRuneVictory(); else{ ai.style.borderColor="red"; setTimeout(()=>ai.style.borderColor="rgba(200,160,80,0.4)",1000); screenShakeHard() } }; as.appendChild(ai); as.appendChild(sb); ov.appendChild(as)
    const br=document.createElement("div"); br.style.cssText="display:flex;gap:10px;margin-top:16px;"; const cb=document.createElement("button"); cb.innerText="💾 Fermer"; cb.style.cssText="padding:8px 20px;font-family:'Cinzel',serif;font-size:12px;background:rgba(20,80,20,0.5);color:#aaffaa;border:1px solid rgba(40,160,40,0.5);border-radius:4px;cursor:pointer;"; cb.onclick=()=>{ const o=document.getElementById("runeChallengeOverlay"); if(o) o.remove() }; br.appendChild(cb); const xb=document.createElement("button"); xb.innerText="✕ Terminer"; xb.style.cssText="padding:8px 20px;font-family:'Cinzel',serif;font-size:12px;background:rgba(80,20,20,0.4);color:#ff8080;border:1px solid rgba(180,40,40,0.4);border-radius:4px;cursor:pointer;"; xb.onclick=closeRuneChallenge; br.appendChild(xb); ov.appendChild(br) }
...
function showAuroraEvent() {
  ...
  fadeMusicOut(()=>{ const mA=document.getElementById("musicA"); if(mA){ mA.src="aurore.mp3"; mA.loop=true; mA.volume=0; mA.currentTime=0; mA.play().catch(()=>{}); let iv=setInterval(()=>{ if(mA.volume<0.35) mA.volume=Math.min(0.35,mA.volume+0.03); else clearInterval(iv) },100); currentMusic="A" } })
  setTimeout(()=>{ msg.style.opacity="0"; setTimeout(()=>msg.remove(),2000) },5000)
  setTimeout(()=>{ ov.style.transition="opacity 4s ease"; ov.style.opacity="0"; setTimeout(()=>{ ov.remove(); auroraActive=false; db.ref("events/aurora").remove(); if(currentMap&&mapMusic[currentMap]) crossfadeMusic(mapMusic[currentMap]) },4000) },55000)
}
...
function doBifrostFlash() {
  fadeMusicOut(()=>{}); const tremb=new Audio("tremblement.mp3"); tremb.volume=0.8; tremb.play().catch(()=>{}); setTimeout(()=>{ let iv=setInterval(()=>{ if(tremb.volume>0.04) tremb.volume-=0.05; else{ tremb.pause(); clearInterval(iv) } },100) },4500)
  const snd=new Audio("bifrost.mp3"); snd.volume=1.0; snd.play().catch(()=>{})
...
function renderMapElement(data) {
  ...
  else{ const img=document.createElement("img"); img.src=""+data.image; img.style.cssText="width:100%;height:100%;object-fit:contain;filter:drop-shadow(0 4px 12px rgba(0,0,0,0.8));pointer-events:none;"; el.appendChild(img) }
...
function renderWantedPoster(data) { const list=document.getElementById("wantedList"); if(!list) return; const card=document.createElement("div"); card.id="wantedCard_"+data.id; card.style.cssText="display:flex;align-items:center;gap:8px;padding:8px;background:rgba(60,40,10,0.4);border:1px solid rgba(150,100,30,0.4);border-radius:4px;"; const img=document.createElement("img"); img.src=""+data.mob+".png"; img.style.cssText="width:36px;height:36px;object-fit:contain;border-radius:3px;"; img.onerror=()=>img.style.opacity="0.3"; card.appendChild(img); const info=document.createElement("div"); info.style.cssText="flex:1;"; info.innerHTML=`<div style="font-family:Cinzel,serif;font-size:11px;color:rgb(255,200,80);">${data.mob.toUpperCase()}</div><div style="font-size:10px;color:rgb(200,160,60);">💰 ${data.reward} po — ${data.tier}</div>`; card.appendChild(info); const del=document.createElement("button"); del.style.cssText="padding:2px 8px;font-size:10px;background:rgba(80,20,0,0.5);color:#ff8888;border:1px solid rgba(150,40,0,0.4);border-radius:3px;cursor:pointer;"; del.innerText="✕"; del.onclick=()=>{ db.ref("game/wantedPosters/"+data.id).remove(); db.ref("elements/"+data.id).remove(); card.remove() }; card.appendChild(del); list.appendChild(card) }
function showWantedOverlay(data) { const ov=document.createElement("div"); ov.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.8);display:flex;align-items:center;justify-content:center;z-index:99999999;cursor:pointer;"; ov.onclick=()=>ov.remove(); const p=document.createElement("div"); p.style.cssText="position:relative;width:300px;padding:30px 20px;text-align:center;"; const bg=document.createElement("img"); bg.src="wanted.png"; bg.style.cssText="position:absolute;top:0;left:0;width:100%;height:100%;object-fit:fill;opacity:0.9;"; p.appendChild(bg); const inner=document.createElement("div"); inner.style.cssText="position:relative;z-index:1;padding:20px;"; const mi=document.createElement("img"); mi.src=""+data.mob+".png"; mi.style.cssText="width:100px;height:100px;object-fit:contain;border:3px solid rgb(100,60,10);border-radius:4px;margin:10px auto;display:block;"; inner.appendChild(mi); const n=document.createElement("div"); n.style.cssText="font-family:'Cinzel Decorative',serif;font-size:18px;color:rgb(80,40,0);letter-spacing:3px;margin-bottom:8px;"; n.innerText=data.mob.toUpperCase(); inner.appendChild(n); const r=document.createElement("div"); r.style.cssText="font-family:Cinzel,serif;font-size:22px;color:rgb(120,70,0);font-weight:bold;"; r.innerText="💰 "+data.reward+" po"; inner.appendChild(r); p.appendChild(inner); ov.appendChild(p); document.body.appendChild(ov) }
...
function triggerCemeteryEvent() {
  ...
  const g=document.createElement("div"); g.id="glipheOverlay"; g.style.cssText="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.88);display:flex;align-items:center;justify-content:center;z-index:99999990;opacity:0;transition:opacity 1s ease;"; const img=document.createElement("img"); img.src="gliphe.png"; img.style.cssText="max-height:70vh;max-width:70vw;object-fit:contain;filter:drop-shadow(0 0 30px purple);"; g.appendChild(img); document.body.appendChild(g); setTimeout(()=>g.style.opacity="1",50)
  db.ref("game/cemeterySpell").set({ active:true, time:Date.now() })
  const spell=new Audio("spell.mp3"); spell.volume=0.9; spell.play().catch(()=>{}); const tremb=new Audio("tremblement.mp3"); tremb.volume=0.7; tremb.play().catch(()=>{})
  ...
  const launch = () => {
    ...
    setTimeout(() => {
      ...
      if (!p) {
        ...
        p.src = "sortprison.mp3"; p.volume = 0
...
function rollSpellDice(playerId, currentTries) {
  const btn=document.querySelector("#spellMiniGame [style*='LANCER']"); if(btn) btn.style.pointerEvents="none"
...
function openAllyPNJPanel() {
  ...
  db.ref("combat/usedAllies").once("value", snap => {
    ...
    ALLY_PNJS.forEach(pnj => {
      ...
      const img = document.createElement("img"); img.src = ""+pnj.image; img.style.cssText = `width:40px;height:40px;border-radius:50%;border:2px solid ${pnj.color};object-fit:contain;box-shadow:0 0 10px ${pnj.color}44;`; img.onerror=()=>img.style.opacity="0.3"
...
function _allyChooseTarget(pnj, action, panel) {
  ...
  ;["greg","ju","elo"].forEach(pid => {
    ...
    btn.innerHTML = `<img src="${pid}.png" style="width:20px;height:20px;border-radius:50%;vertical-align:middle;margin-right:8px;">${pid.toUpperCase()}`
...
function _allyInvocationCinematic(pnj, action, targetId) {
  ...
  setTimeout(()=>{
    ...
    // Image du dieu
    const img = document.createElement("img"); img.src = ""+pnj.image
    ...
    const impact = new Audio("impact.mp3"); impact.volume = 0.85; impact.play().catch(()=>{})
...
function _rollAllyDice(pnj, action, targetId) {
  ...
  const spinIv = setInterval(()=>{
    ...
    if (spins >= maxSpins) {
      ...
      // Son diceinv.mp3
      const diceInv = new Audio("diceinv.mp3"); diceInv.volume = 0.85; diceInv.play().catch(()=>{})
...
function openAllyPNJViewer() {
  ...
  db.ref("combat/usedAllies").once("value", snap => {
    ...
    ALLY_PNJS.forEach(pnj => {
      ...
      const img = document.createElement("img"); img.src = ""+pnj.image; img.style.cssText = `width:36px;height:36px;border-radius:50%;border:1px solid ${pnj.color}66;object-fit:contain;filter:grayscale(20%);`; img.onerror=()=>img.style.opacity="0.3"
...
function toggleGoldInput() {
  ...
  const snd = new Audio("coin.mp3"); snd.volume = 0.7; snd.play().catch(()=>{})
